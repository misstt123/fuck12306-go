...
func SubmitOrderRequest(cookieText string, secretStr string, traindate string, from string, to string) {
	now := time.Now()
	url := "https://kyfw....cn/otn/leftTicket/submitOrderRequest"
	method := "POST"
...
func CheckOrderInfo(cookieText string, zw string, ccr []string) {
	...
	url := "https://kyfw....cn/otn/confirmPassenger/checkOrderInfo"
	method := "POST"
	u := ""
	o := ""
	s := city.Seat(zw)
...
func ConfirmSingleForQueue(cookieText string, u string, o string) {
	...
	url := "https://kyfw....cn/otn/confirmPassenger/confirmSingleForQueue"
	method := "POST"
	...
	if rq.Status {
		...
		_ = notice.SendMail(c.Mail, "恭喜抢票成功！", "抢票成功快登录..付款30分钟内有效")
